...
class Driver {
    ...
    _onExportsPair (socket) {
        ...
        socket.on('authenticate', (data, callback) => {
            if (Homey.manager('settings').get('auth')) {
    ...
    isFirmwareChanged (device_data, callback) {
        ...
        if (device.info.firmware_version !== device_data.firmware_version) {
            let tokens = {
                'old_firmware': device.info.firmware_version,
                'new_firmware': device_data.firmware_version
            }

            Homey.manager('flow').triggerDevice(this._deviceType + '_firmware_changed', tokens, null, device.data, (error, result) => {
                if (error) {
